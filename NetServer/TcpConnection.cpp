	...
	bufferout_()
{
    ...
    spchannel_->SetReadHandle(std::bind(&TcpConnection::HandleRead, this));//回调！回调！注册到channal中，channel遇到对应的情况时候，就回调tcpconnetion提供的回调函数
    spchannel_->SetWriteHandle(std::bind(&TcpConnection::HandleWrite, this));//tcpconnection的回调是来自tcpserver，tcp
...
void TcpConnection::AddChannelToLoop()
{
	...
	loop_->AddTask(std::bind(&EventLoop::AddChannelToPoller, loop_, spchannel_.get()));//自能
...
void TcpConnection::SendInLoop()
{
    ...
    if(result > 0)
    {
        ...
        if(bufferout_.size() > 0)
        {
			...
			loop_->UpdateChannelToPoller(spchannel_.get());//epollout事件的到来会回调handlewrite，此处就是et模式下发送数据的处理
        }
		else
		{
			//数据已发完
...
void TcpConnection::HandleRead()
{
    ...
    if(result > 0)
    {
        messagecallback_(shared_from_this(), bufferin_);//可以用右值引用优化//读入之后，就回调应用层的消息处理函数，粘包问题的处理都是交给应用层协议处理的。
		//bufferin_.clear();
    }
    else if(result == 0)
    {
...
int recvn(int fd, std::string &bufferin)
{
    ...
    for(;;)
    {
    	...
    	if (nbyte > 0)
		{
            bufferin.append(buffer, nbyte);//效率较低，2次拷贝,//直接用string的append方法，加入到缓冲区,
            readsum += nbyte;
			if(nbyte < BUFSIZE)//??????
		...
		{
			if (errno == EAGAIN)//系统缓冲区未有数据，非阻塞返回//读到eagin为止
			{
...
int sendn(int fd, std::string &bufferout)
{
	...
	for (;;)//循环发送，要么全发送完毕，要么发送到eagin
	{
		...
		if (nbyte > 0)
		{
			...
			if (length == 0 )
			{
				return sendsum;//et模式下的发送，要么就是把应用层缓冲区的数据都怼到内核的发送缓冲区去，要么就是一直怼，直到出现eagain事件，而且，此缓冲区的设置用
				//string来做，只要把某一部分数据从应用层缓冲区搬运到内核缓冲去，string的游标就会向右移动。
			}
		}
		else if (nbyte < 0)//异常
		{
