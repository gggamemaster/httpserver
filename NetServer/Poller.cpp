...
Poller::Poller(/* args */)
    ...
    mutex_()
{
    pollfd_ = epoll_create(256);//epoll的第一个api
    if(pollfd_ == -1)
    {
...
Poller::~Poller()
{
    close(pollfd_);//关闭epoll对应的描述符
}

//等待I/O事件
void Poller::poll(ChannelList &activechannellist)//找出活跃的通道列表
{
    int timeout = TIMEOUT;
    //std::cout << "epoll_wait..." << std::endl;(int)eventlist_.capacity()
    int nfds = epoll_wait(pollfd_, &*eventlist_.begin(), (int)eventlist_.capacity(), timeout);// int epoll_wait(int epfd, struct epoll_event * events, int maxevents, int timeout);,maxevent告知数组大小
    ...
    if(nfds == (int)eventlist_.capacity())//扩容
    {
